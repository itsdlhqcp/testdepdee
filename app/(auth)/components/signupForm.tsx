...
// import { registerUser, checkEmailAvailability } from "@/serverActions/auth";
...
//   // Watch email field and debounce it
...
export function SignUpForm() {
  ...
  useEffect(() => {
    const checkPhone = async () => {
      ...
      if (!debouncedPhone || debouncedPhone.length < 13) {
  ...
  const onSubmit = (data: Inputs) => {
    ...
    // Prevent submission if phone is not available
    if (phoneCheckStatus.available === false) {
    ...
    >
      ...
      <button
        disabled={
          ...
          emailCheckStatus.available === false ||
          emailCheckStatus.checking ||
          phoneCheckStatus.available === false ||
          phoneCheckStatus.checking
        }
        type="submit"
        className="w-full rounded-[12px] h-[50px] bg-brand text-white font-medium transition-all duration-200 hover:bg-orange-500 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center gap-2"
